...
void clear_info(char *name_file, char *info)
{
  char buffer[MAXBUFF];
  FILE *lin;
  int i;
...
void dead_messages(char *name_file, char *username, char *tty, char *ip, char *hostname)
{
...
void dead_uwbtmp(char *name_file, char *username, char *tty)
{
  struct utmp pos;
  int fd;
  ...
  {
    if ( ( strncmp( pos.ut_name, username, sizeof(pos.ut_name) ) == 0) && ( strncmp( pos.ut_line, tty, sizeof(pos.ut_line) ) == 0) ) {      
...
void dead_lastlog(char *name_file, char *username)
{
  struct passwd *pwd;
  struct lastlog pos;
  int fd;
